...
const technologySchema = Joi.object({
  ...
  systemType: Joi.string().optional().max(100)
    .description('Very short generic name which describes the essential purpose of the system '),
...
This is intended to give a rough idea of the system "size" through user numbers. `),
  commsResponsibility: Joi.string().optional().max(100)
    .description('Where does responsibility for communications concerning system and process change reside? e.g. department (which one)'),
  integrationResponsibility: Joi.string().optional().max(100)
    .description('Who develops the code for system integrations e.g. department (which one), vendor, third party?Where does responsibility and funding for licence renewals reside? E.g. central budget, departmental budget (which dept/role/person)'),
  licensingResponsibility: Joi.string().optional().max(100)
    .description('Where does responsibility and funding for licence renewals reside? E.g. central budget, departmental budget (which dept/role/person)'),
  reportWriting: Joi.string().optional().max(100)
    .description('Who is responsible for developing operational reporting from the system e.g. department (which one), vendor, users'),
  userAccessControl: Joi.string().optional().max(100)
    .description('Who is responsible for granting access setting up roles/role groups etc? e.g. department/job role (not a named person)'),
  trainingDelivery: Joi.string().optional().max(100)
    .description('Who delivers training to end users e.g. department, vendor, third party?'),
  vendorManagement: Joi.string().optional().max(100)
    .description('Who is responsible for vendor relationship management e.g. regular meetings, point of contact for new feature descriptions, management of support contract etc. e.g. department (which one)'),
  ...
  incidentsAndProblems: Joi.string().optional().max(100)
    .description('Where are incident and problem management executed e.g.department (which one), vendor, third party?'),
  ...
  enhancements: Joi.string().optional().max(100)
    .description('Who does the work in delivering application enhancements (build or configure) e.g. department (which one), vendor, third party?'),
  upgradeDecisions: Joi.string().optional().max(100)
    .description('Who is responsible for decisions around upgrade dates, feature adoption etc. e.g. department/job role (not named person)'),
  upgradeFundingSource: Joi.string().optional().max(100)
    .description('How are major upgrades and/or enhancements funded e.g. Operational budget, Departmental budget, none defined'),
  upgradeImplementation: Joi.string().optional().max(1024)
    .description('Who is responsible for doing and/or organising the work in app patching, minor upgrades, major upgrades e.g. department (which one), vendor. Note that the work may be organised by the owning department but actually done by another party, such as the vendor'),
