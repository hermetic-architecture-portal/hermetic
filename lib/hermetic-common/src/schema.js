...
const link = Joi.object({
  ...
  text: Joi.string().optional().max(70)
...
const capabilitySchema = Joi.object({
  ...
  valueChainSegments: Joi.array().items({
    ...
    valueChainSegmentId: Joi.string()
      .fk('valueChains.[].valueChainSegments.[].valueChainSegmentId', { parentFieldName: 'valueChainId' })
      .required().pk(),
...
const technologySchema = Joi.object({
  ...
  technologyType: Joi.string().optional()
    .description('Icons are available for the following values: Client/server, SaaS, Server only, Support service, Platform.\nAny other value can be used but will not be mapped to an icon.'),
...
const networkNodeSchema = Joi.object({
  ...
  isAbstractNode: Joi.boolean().optional()
    .description('Abstract nodes represent a server role (e.g. "Domain controller") rather than an actual individual server'),
  nodeType: Joi.string().optional()
    .description('Free text node type category.\nKnown values with mapped icons are: Database, Desktop, Mobile Device, Cluster, Load Balancer, Load Balancer Alias, Server'),
...
const metric = Joi.object({
  metricId: Joi.string().required().pk(),
  name: Joi.string().required().max(70).meta({ displayName: true }),
  description: Joi.string().required().max(1024),
  category: Joi.string().required()
    .description('Category to group metrics into'),
  minScore: Joi.number().required()
    .description('Scoring assumes higher = better, even if the metric description is negative (e.g. Technical Debit)'),
...
const technologyMetricAssessmentSet = Joi.object({
  ...
  assessments: Joi.array().required().uniqueOnPks().items(metricAssessment),
});
...
const componentConnection = Joi.object({
  ...
  toInterfaceId: Joi.string()
    .fk('components.[].interfaces.[].interfaceId', { parentFieldName: 'toComponentId' })
    .optional().pk()
    .meta({ displayName: true })
...
const schema = Joi.object({
  ...
  capabilityMetricSets: Joi.array().items(metricSet).optional().uniqueOnPks()
    .description('Capability metric sets define different ways of measuring technologies'),
